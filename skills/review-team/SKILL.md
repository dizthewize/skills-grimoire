...
| `issue` | auto-detect | GitHub issue number for acceptance criteria context (e.g., `issue=123` or `issue=#123`) |
| `ticket` | auto-detect | Legacy Jira ticket key override (e.g., `ticket=PROJ-94`) — use `issue=` for GitHub projects |
...
/review-team 123
/review-team 123 issue=456            # attach GitHub issue for acceptance criteria context
/review-team 123 ticket=PROJ-94       # legacy Jira ticket key
...
4. **Issue/ticket auto-detection**: Resolve issue context for acceptance criteria validation using this priority order:
   ...
   **b) `ticket=` parameter provided (Jira)**: Fetch via Jira MCP:
   ...
   - Extract: `key`, `fields.summary`, `fields.description`, `fields.customfield_10014` (acceptance criteria)

   **c) Auto-detect from PR**: Scan PR title and branch name for:
   - GitHub issue patterns: `#\d+`, `closes-\d+`, `fix-\d+` — fetch via `gh issue view`
   - Jira patterns: `{PROJECT_KEY}-\d+` (e.g., `PROJ-123`) — fetch via Jira MCP if available

   Non-blocking if no issue/ticket found or fetch fails — continue without that context. QA reviewer will note "No acceptance criteria available".
